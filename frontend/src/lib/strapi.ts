...
export const DEMO_ARTICLES: Article[] = [
  {
    id: 1,
    title:
      "Panduan Praktis Setup Docker & Cloudflare Tunnel untuk Self-Hosting",
    slug: "panduan-setup-docker-cloudflare-tunnel",
    excerpt:
      "Cara aman dan hemat biaya menjalankan web server dan CMS sendiri dari rumah tanpa membuka port router fisik.",
    readingTime: 4,
    ...
    content: [
      ...
      },
      {
        ...
        caption:
          "Penjelasan cara kerja Cloudflare Zero Trust & Cloudflared Tunnel",
      },
      {
        type: "heading",
        level: 2,
...
services:
  ...
  },
  {
    ...
    excerpt:
      "Eksplorasi hobi keyboard mekanikal: memilih layout 65% vs 75%, jenis switch (linear, tactile), hingga teknik lubing.",
    readingTime: 5,
    ...
    content: [
      ...
      },
      {
        ...
        caption:
          "Contoh sound test tactile switch setelah dilube dengan Krytox 205g0",
      },
      {
        type: "callout",
        calloutType: "info",
  ...
  },
  {
    id: 3,
    title:
      "Mengenal Bun 1.x: Mengapa Ekosistem JavaScript Bergerak Semakin Cepat",
    slug: "mengenal-bun-javascript-runtime-cepat",
    excerpt:
      "Ulasan arsitektur runtime Bun, kompatibilitas Node.js, dan bagaimana Bun menghemat waktu build dalam pipeline CI/CD.",
    readingTime: 3,
...
export const getArticles = cache(async function getArticles(options?: {
  isDraft?: boolean;
}): Promise<Article[]> {
  try {
    ...
    return (data.data as StrapiArticleItem[]).map((item) => ({
      ...
      content:
        item.better_content &&
        (Array.isArray(item.better_content)
          ? item.better_content.length > 0
          : true)
          ? item.better_content
          : item.content || [],
      ...
      category: item.category
        ...
        : undefined,
      tags:
        item.tags?.map((t) => ({ id: t.id, name: t.name, slug: t.slug })) || [],
      author: item.author
        ? {
            id: item.author.id,
            ...
            avatarUrl: item.author.avatar?.url
              ? formatCoverUrl(item.author.avatar.url)
              : undefined,
...
export async function getAdjacentArticles(currentSlug: string): Promise<{
  ...
  const prev =
    currentIndex < articles.length - 1 ? articles[currentIndex + 1] : null; // Lebih lama
...
export async function getAuthorByIdOrSlug(
  ...
  const matchedArticle = articles.find(
    (a) =>
      a.author &&
      (String(a.author.id) === decoded ||
        slugifyAuthor(a.author.name) === decoded),
  );
...
export async function getArticlesByTag(tagSlug: string): Promise<Article[]> {
  ...
  return articles.filter((a) =>
    a.tags?.some((t) => t.slug.toLowerCase() === decoded),
  );
}
